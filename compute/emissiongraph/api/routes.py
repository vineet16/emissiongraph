...
async def query_llm(req: QueryRequest):
    ...
    system = (
        ...
        "## How to reason about emissions data\n\n"
        "ALWAYS answer the WHY, not just the WHAT. Every observation has an operational cause "
        "visible in the data. Your job is to find it.\n\n"
        "1. DECOMPOSE: Use the per-MT breakdown (kg CO2e/MT) to identify which sources drive "
        "any pattern. These sum to total intensity — compare them across ports.\n"
        "2. EXPLAIN CAUSES: When a source is high or low, look at what explains it:\n"
        "   - Near-zero electricity emissions? Check if the port uses renewable/captive power.\n"
        "   - Near-zero diesel? Look at what dominates instead — is it an electrified port?\n"
        "   - High HFHSD/IFO? That's operational waste fuel — a port-specific activity.\n"
        "   - Always trace the 'why' one level deeper than the number itself.\n"
        "3. COMPARE FULLY: For comparison questions, build a complete table of ALL sources "
        "(portA kg/MT vs portB kg/MT, diff, % of gap) before writing the narrative. "
        "Include every source above 0.01 kg/MT difference. Note offsetting sources.\n"
        "4. DISCLOSE OPERATIONAL DIFFERENCES: Renewable vs grid electricity, electrified vs "
        "fuel-dependent operations, presence/absence of specific fuel types — these are the "
        "insights that matter. Never say 'alternative energy' when the data shows it's "
        "grid electricity or renewable power specifically.\n"
        "5. BE SPECIFIC: Use exact numbers. '99.99% of emissions are Scope 2 electricity' is "
        "better than 'relies heavily on electricity'. Quantify percentages of gaps.\n\n"
